...
DEFAULT_SHAZAM_APP_PORT = "5001"
DEFAULT_SHAZAM_APP_SCHEME = "https"


def _shazam_app_url():
    """Return the configured URL for the separate Shazam project."""
    configured_url = os.environ.get("SHAZAM_APP_URL", "").strip()

    if configured_url:
        return configured_url

    host = request.host.split(":", 1)[0] if request.host else "127.0.0.1"
    scheme = os.environ.get("SHAZAM_APP_SCHEME", DEFAULT_SHAZAM_APP_SCHEME).strip() or DEFAULT_SHAZAM_APP_SCHEME
    port = os.environ.get("SHAZAM_APP_PORT", DEFAULT_SHAZAM_APP_PORT).strip() or DEFAULT_SHAZAM_APP_PORT

    return f"{scheme.rstrip(':/')}://{host}:{port}"


@app.context_processor
def inject_navigation_links():
    """Expose cross-project navigation links to every rendered page."""
    return {"shazam_app_url": _shazam_app_url()}
